...
style = u'''<style type="text/css">
    ...
    ins { background-color: rgb(153, 238, 153); }
    pre {
...
def to_html(changes):
    ...
    for chg in changes:
        #pprint(chg)
        if chg['type'] == 'unmod':
            _add(chg['base']['lines'], 'unmod')
        elif chg['type'] == 'mod':
            _add(chg['base']['lines'], 'rem')
            _add(chg['changed']['lines'], 'add')
        elif chg['type'] == 'add':
            _add(chg['changed']['lines'], 'add')
        elif chg['type'] == 'rem':
            _add(chg['base']['lines'], 'rem')
...
def generate_html_diff(input_fn, output_fn):

    d1 = codecs.open(input_fn, 'rb', 'utf-8').read()
    d2 = codecs.open(output_fn, 'rb', 'utf-8').read()
    ...
    return to_html(changes).encode('utf-8')


if __name__ == "__main__":
    import sys
    html_content = generate_html_diff(sys.argv[1], sys.argv[2])
